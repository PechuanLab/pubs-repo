...
cites =  abs$name[8:16]
# Read data
data = Read10X(data.dir = "SAM24427128_CellRanger/")
data_hto = data$`Antibody Capture`[htos,]
data_adt = data$`Antibody Capture`[cites,]
# Change ADTs Rownames
rownames(data_hto) = c("II_1","II_2","IE_2","EE_2","EE_1","IE_1")
...
# Add experiment info
seu@meta.data$Experiment = "SortedDCs"
...
# 
seu@meta.data$hash.ID = factor(seu@meta.data$hash.ID, levels = c("EE-1","EE-2","IE-1","IE-2","II-1","II-2") ) 
HashIDPalette = c("firebrick","firebrick","darkgoldenrod1","darkgoldenrod1","forestgreen","forestgreen")
DimPlot(seu, group.by = "hash.ID", cols = HashIDPalette)+ theme_pubr(base_size = 10) +
  scale_color_manual(labels = paste(levels(seu@meta.data$hash.ID),table(seu@meta.data$hash.ID),sep=": "),
                     values=HashIDPalette)
table(seu@meta.data$hash.ID)


seu = NormalizeData(seu, assay = "CiteSeq", normalization.method = "CLR",verbose = T)
abs
FeaturePlot(seu,"Irf8")
...
SaveH5Seurat(seu, filename = "SAM24427128.h5Seurat")
Convert("SAM24427128.h5Seurat", dest = "h5ad")
saveRDS(seu,"SAM24427128.rds")
